...
public class OverScrollListView extends ListView {
	public final static int DEFAULT_MAX_OVER_SCROLL_DURATION = 350;
	public final static int DEFAULT_FINISH_DELAYED_DURATION = 1000;

	// boucing for a normal touch scroll gesture(happens right after the finger
	// leaves the screen)
	...
	public void setPullToRefreshHeaderView(View headerView) {
		...
		if (!(headerView instanceof IPullToRefreshCallback)) {
			throw new IllegalArgumentException(
					"Pull-to-refresh header view must implement PullToRefreshCallback");
		}

		mOrigHeaderView = (IPullToRefreshCallback) headerView;

		if (headerView instanceof ViewGroup) {
			mHeaderView = ((ViewGroup) headerView).getChildAt(0); // pay
																	// attention
																	// to this
			if (mHeaderView == null
					|| (!(mHeaderView instanceof LinearLayout) && !(mHeaderView instanceof RelativeLayout))) {
				throw new IllegalArgumentException(
						"Pull-to-refresh header view must have "
		...
		try {
			...
			if (listAdapter instanceof HeaderViewListAdapter) {
				listAdapter = ((HeaderViewListAdapter) listAdapter)
						.getWrappedAdapter();
				throw new RuntimeException(e.getMessage() + ", adapter=["
						+ listAdapter.getClass() + "]", e);
			}
			throw e;
		}
	}
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		if (mHeaderViewHeight == 0 && mHeaderView != null) {
			...
			// after the first "laying-out", we get the original height of
			// header view
			...
			if (mMarkAutoRefresh) {
				...
				post(new Runnable() {
					@Override
					public void run() {
						startRefreshingManually(mBizContextForRefresh);
					}
				});
			} else {
				// set the header height to 0 in advance. "post(Runnable)" below
				// is queued up
	...
	public void setPullToLoadMoreFooterView(View footerView) {
		if (!(footerView instanceof IPullToLoadMoreCallback)) {
			throw new IllegalArgumentException(
					"Pull-to-load-more footer view must implement PullToLoadMoreCallback");
		}

		mSavedFooterView = (IPullToLoadMoreCallback) footerView;
	...
	public void finishRefreshingSuccess(Runnable runnable) {
		finishRefreshingSuccess(runnable, DEFAULT_FINISH_DELAYED_DURATION);
	}

	public void finishRefreshingSuccess(long delayedDuration) {
		finishRefreshingSuccess(new Runnable() {
	...
	public void finishRefreshingSuccessByDefaultDelay() {
		finishRefreshingSuccess(DEFAULT_FINISH_DELAYED_DURATION);
	}

	public void finishRefreshingSuccess(final Runnable runnable,
			long delayedDuration) {
		if (mOrigHeaderView != null) {
			mOrigHeaderView.onFinishRefreshingSuccess();
		}
	...
	public void finishRefreshingFailed(Runnable runnable) {
		finishRefreshingFailed(runnable, DEFAULT_FINISH_DELAYED_DURATION);
	}

	public void finishRefreshingFailed(long delayedDuration) {
		finishRefreshingFailed(new Runnable() {
	...
	public void finishRefreshingFailedByDefaultDelay() {
		finishRefreshingFailed(DEFAULT_FINISH_DELAYED_DURATION);
	}

	public void finishRefreshingFailed(final Runnable runnable,
			long delayedDuration) {
		if (mOrigHeaderView != null) {
			mOrigHeaderView.onFinishRefreshingFailed();
		}
	...
	public void enablePullRefresh(boolean enable) {
		if (enable) {
			if (mSavedHeaderView != null) {
				mOrigHeaderView = mSavedHeaderView;
				((View) mSavedHeaderView).setVisibility(VISIBLE);
			}
		} else if (mOrigHeaderView != null) {
			((View) mOrigHeaderView).setVisibility(GONE);
			mSavedHeaderView = mOrigHeaderView;
	...
	@SuppressLint("NewApi") protected boolean overScrollBy(int deltaX, int deltaY, int scrollX,
			int scrollY, int scrollRangeX, int scrollRangeY,
			int maxOverScrollX, int maxOverScrollY, boolean isTouchEvent) {
		if (!isTouchEvent && mScroller.isFinished()) {
			mVelocityTracker.computeCurrentVelocity(
					(int) (16 * mScreenDensity), mMaximumVelocity);
			int yVelocity = (int) mVelocityTracker.getYVelocity(0);

			if ((Math.abs(yVelocity) > mMinimumVelocity)) {
				mScroller.fling(0, getScrollY(), 0, -yVelocity, 0, 0,
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_CANCEL:
			...
			// 'getScrollY != 0' means that content of the ListView is off
			// screen.
			// Or if it is not in "refreshing" state while height of the header
			// view
			// is greater than 0, we must set it to 0 with a smooth bounce
			// effect
		...
		// if not in 'refreshing' state or scrollY is less than zero, and height
		// of
		// header view is greater than zero. we should keep the the first item
		// of the
		// ListView always at the top(we are decreasing height of the header
		// view, without
		// calling setSelection(0), we will decrease height of the header view
		// and scroll
		// the ListView itself at the same time, which will cause scrolling too
		// fast
		...
		try {
			...
			if (listAdapter instanceof HeaderViewListAdapter) {
				listAdapter = ((HeaderViewListAdapter) listAdapter)
						.getWrappedAdapter();
				throw new IllegalStateException(e.getMessage() + ", adapter=["
						+ listAdapter.getClass() + "]", e);
			}
			throw e;
		}
	...
	private void handleTouchScroll(int deltaY) {
		...
		// 0.4f is just a number that gives OK effect out of many tests. it
		// means nothing special
		float scale = ((float) listViewHeight - Math.abs(scrollY) - getCurrentHeaderViewHeight())
				/ getHeight() * 0.4f;
		...
		if (reachTopEdge) {
			...
			if (deltaY > 0) {
				if (scrollY > 0) {
					// when scrollY is greater than 0, it means we reach the
					// bottom of the list
					// and the ListView is scrolled off the screen from the
					// bottom, now we
					// scrollDown() to scroll it back, otherwise, we just let
	...
	private void springBack(int scrollY) {
		mScroller.startScroll(0, scrollY, 0, -scrollY,
				DEFAULT_MAX_OVER_SCROLL_DURATION);
		postInvalidate();
	}
	...
	public void computeScroll() {
		if (mScroller.computeScrollOffset()) {
			...
			// header view to 0
			if (!mHideHeaderViewWithoutAnimation && !mIsRefreshing
					&& getCurrentHeaderViewHeight() > 0) {
			...
			postInvalidate();
				...
				&& (getScrollY() != 0 || (!mIsRefreshing && getCurrentHeaderViewHeight() != 0))) {
			springBack();
		}
	 ...
	 * 
	 * 1. check if height of the header view is greater than 0, if so, decrease
	 * it to 0
	 * 
	 * 2. scroll content of the ListView off the screen any there's any deltaY
	 * left(i.e. deltaY is not 0)
	 */
	 ...
	 * 
	 * 1. if scrollY is less than 0, it means we have scrolled the list off the
	 * screen from the top, now we scroll back and make the list to reach the
	 * top edge of the screen.
	 * 
	 * 2. check height of the header view and see if it is greater than 0, if
	 * so, we decrease it and make it zero.
	 * 
	 * 3. now check if we have scrolled the list to reach the bottom of the
	 * screen, if so we scroll the list off the screen from the bottom.
	 */
	...
	public void scrollTo(int x, int y) {
		...
		if (mOrigHeaderView != null && y < 0 && !mIsRefreshing) {
			...
			if (y > halfPullDistanceThreshold) {
				...
				} else if (oldScrollY < mLoadingMorePullDistanceThreshold
						&& y >= mLoadingMorePullDistanceThreshold) {
	...
	private void setHeaderViewHeight(int height) {
		if (mHeaderViewLayoutParams != null
				&& (mHeaderViewLayoutParams.height != 0 || height != 0)) {
			setHeaderViewHeightInternal(height);
		}
	}

	/* package */void setHeaderViewHeightInternal(int height) {
		int oldHeight = mHeaderViewLayoutParams.height;

		mHeaderViewLayoutParams.height = height;

		// if mHeaderView is visible(I mean within the confines of the visible
		// screen), we should
		// request the mHeaderView to re-layout itself, if mHeaderView is not
		// visible, we should
		// redraw the ListView itself, which ensures correct scroll position of
		// the ListView.
		...
		if (mOrigHeaderView != null && !mIsRefreshing && !mCancellingRefreshing) {
			...
			} else if (oldHeight == mHeaderViewHeight
