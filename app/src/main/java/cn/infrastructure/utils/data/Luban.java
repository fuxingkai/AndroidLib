 ...
 * 分为一档压缩与三档压缩
 *使用：
 * Luban.get(GoHomeApplication.getInstance()).load(files).putGear(Luban.FIRST_GEAR).asFilesObservable().subscribeOn(Schedulers.io())
 ...
 * .onErrorResumeNext(new Func1<Throwable, Observable<? extends ArrayList<File>>>() {
 *@Override
 *public Observable<? extends ArrayList<File>> call(Throwable throwable) {return Observable.empty();}})
 *.subscribe(new Action1<ArrayList<File>>() {
 *@Override
 *public void call(ArrayList<File> files) {// TODO 压缩成功后调用，返回压缩后的图片文件
 * sendLifeCircleMomentCreateReq(lifeCircleMomentCreateReq, files, callback);}});
 *
 * Created by Frank on 2016/10/13.
 */
import static cn.infrastructure.utils.data.Preconditions.checkNotNull;
...
public class Luban {
    ...
    public Luban load(ArrayList<File> files) {
        mFiles = files;
        return this;
    }
    ...
    public Observable<File> asFileObservable() {
        if (gear == FIRST_GEAR) {
    ...
    public Observable<ArrayList<File>> asFilesObservable() {
        if (gear == FIRST_GEAR)
            return Observable.just(mFiles).map(new Func1<ArrayList<File>, ArrayList<File>>() {
                @Override
                public ArrayList<File> call(ArrayList<File> files) {
                    ArrayList<File> compressFiles = new ArrayList<File>();
                    for (File file : files) {
                        Luban.this.filename=file.getName();
                        compressFiles.add(firstCompress(file));
                    }
            ...
            return Observable.just(mFiles).map(new Func1<ArrayList<File>, ArrayList<File>>() {
                ...
                public ArrayList<File> call(ArrayList<File> files) {
                    ...
                    for (File file : files) {
                        Luban.this.filename=file.getName();
                        compressFiles.add(thirdCompress(file));
                    }
                    return compressFiles;
                }
            });
